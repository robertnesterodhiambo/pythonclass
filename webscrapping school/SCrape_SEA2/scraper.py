...
def open_website():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)  # Set headless=False for debugging, True for silent execution
        ...
        for value in df["sea_combined"].head(6):
            try:
                ...
                # Extract the unit number text
                unit_number = page.locator("#__view1-__clone1").text_content().strip()
                
                # Print extracted data
                print(f"Processed Entry: {value}, Extracted Unit Number: {unit_number}")
                
                # Store in the list
                extracted_data.append({"Input": value, "Unit Number": unit_number})
